...
-- Drop triggers first
DROP TRIGGER IF EXISTS trigger_auto_create_duty_hours ON oncall_shifts;
DROP TRIGGER IF EXISTS update_duty_hours_updated_at ON duty_hours;
DROP TRIGGER IF EXISTS update_swap_requests_updated_at ON shift_swap_requests;
DROP TRIGGER IF EXISTS update_oncall_shifts_updated_at ON oncall_shifts;
DROP TRIGGER IF EXISTS update_oncall_schedules_updated_at ON oncall_schedules;

-- Drop functions
DROP FUNCTION IF EXISTS auto_create_duty_hours();
DROP FUNCTION IF EXISTS get_next_available_date(UUID, UUID, DATE);
DROP FUNCTION IF EXISTS check_80_hour_week_violation(UUID, DATE);
DROP FUNCTION IF EXISTS calculate_shift_hours(TIME, TIME, shift_type);
